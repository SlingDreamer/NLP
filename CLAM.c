...
void SortVocab() {
  ...
  for (a = 0; a < size; a++) {
    ...
    if ((vocab[a].cn < min_count) && (a != 0)) {
      vocab_size--;
        //modification begin
      if(vocab[a].Component != NULL) free(vocab[a].Component);
        //modification end
      free(vocab[a].word);
...
char* GetMainWordOfPhrase(char *str, char spliter){      //获取最长单词
  int idx, maxIdx;
  int len;
  char *dst;
  int cnt;
  ...
  maxIdx = 0;
  len = strlen(tmp[0]);
  for(idx = 0; idx < cnt; idx++){   //获取矩阵最大横长度
  ...
  strcpy(dst, tmp[maxIdx]);   //copy max len data
      ...
      if(wordCnt > 0){
        ...
        for(idx = 0; idx < wordCnt; idx++)
        {
          ...
          if(prefixWord != -1 && prefixWord != 0){
            wordMap[map_size].Component[effCnt].position = prefixWord;
    ...
    if (cbow) {  //train the cbow architecture
      ...
      for (a = b; a < window * 2 + 1 - b; a++){ 
        if (a != window) {
          ...
          if(ComCnt != 0){
            for (c = 0; c < dim; c++)
              morpheme[c] += (ComComp[c]) / sumWeight; //wegihted averaging
            norm = 2;
          }
