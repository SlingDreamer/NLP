...
int AddWordToVocab(char *word) {
  ...
  //modification begin
  vocab[vocab_size].Component = NULL;
  vocab[vocab_size].ComponentNum = 0;
  //modification end
...
void LoadMapData(){
  ...
  for(idx = 0; idx < map_size; idx++){
    ...
    wordMap[idx].ComponentNum = 0;
    wordMap[idx].Component = NULL;
  }
  ...
  while(1){
    ...
    if(0 != strcmp(mPtr[1], " ")){
      ...
      if(wordCnt > 0){
        wordMap[map_size].Component = (struct pos*)malloc(wordCnt * sizeof(struct pos)); //store Component
        effCnt = 0;
        for(idx = 0; idx < wordCnt; idx++)
        {
          char *tmpMainWord = subPtr[idx];
          long long ComponetWord = SearchVocab(tmpMainWord);
          if(ComponetWord != -1 && ComponetWord != 0){
            wordMap[map_size].Component[effCnt].position = ComponetWord;
            wordMap[map_size].Component[effCnt].weight = 1;
            effCnt++;
          }
        }
  ...
  for(vIdx = 1; vIdx < vocab_size; vIdx++){
    ...
    if(wordMap[curIdx].ComponentNum != 0){
      vocab[vIdx].ComponentNum = wordMap[curIdx].ComponentNum;
      vocab[vIdx].Component = wordMap[curIdx].Component;
    }
...
void *TrainModelThread(void *id) {
  ...
  real *ComponentComp = (real *)calloc(dim, sizeof(real));
  ...
  while (1) {
    if (word_count - last_word_count > 10000) {
      ...
      if ((debug_mode > 1)) {
        ...
        // printf("[Debug]%cMy_word_count_actual: %.2f%%  ", 13,word_count_actual);
        fflush(stdout);
      }
    ...
    if (cbow) {  //train the cbow architecture
      ...
      for (a = b; a < window * 2 + 1 - b; a++){ 
        if (a != window) {
          ...
          ComCnt = vocab[last_word].ComponentNum; 
          ...
          if(ComCnt != 0){
            for(curIdx = 0; curIdx < ComCnt; curIdx++){
              ...
              sim = 0;
              for (c = 0; c < dim; c++) {
                len += syn0[c + ComponentWord * dim] * syn0[c + ComponentWord * dim];
              }
              len = sqrt(len);
             ...
             // printf("[Debug] TrainModelThread-Component: %f\n", ComponentWord);
              for (c = 0; c < dim; c++) ComponentComp[c] +=  syn0[c + ComponentWord * dim] * sim;
              ComWeight += sim;
            }
          }
          ...
          for (c = 0; c < dim; c++) neu1[c] += morpheme[c] / norm;
	        //modification end
printf("[Debug] Reach the palce of cbow qianxiang \n");
          cw++;
        }
      }
      if (cw) { // CBOW
        ...
        for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
          ...
          //modification end
printf("[Debug] Reach the palce of cbow gengxin hou \n");
        }
      }
...
void TrainModel() {
  ...
  if (classes == 0) {
    // Save the word vectors
	  ...
	  //modification begin
printf("[Debug] Reach the palce of save 1 \n");
    fprintf(fo, "%lld %lld\n", vocab_size, dim); //disable the input of vocab_size and dim
	  //modification end
printf("[Debug] Reach the palce of save first one \n");
    for (a = 0; a < vocab_size; a++) {
      fprintf(fo, "%s ", vocab[a].word);
printf("[Debug] Reach the palce of save jia vocab.word \n");
      if (binary) for (b = 0; b < dim; b++) fwrite(&syn0[a * dim + b], sizeof(real), 1, fo);
      else for (b = 0; b < dim; b++) fprintf(fo, "%lf ", syn0[a * dim + b]); printf("[Debug] Reach the palce of save dim \n");
      fprintf(fo, "\n");
    }
